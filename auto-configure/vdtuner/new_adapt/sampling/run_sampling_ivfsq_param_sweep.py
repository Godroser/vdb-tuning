#!/usr/bin/env python3
"""Full-factorial sweep for IVF-SQ parameters with sampling benchmark."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep SAMPLE_RATIO x nlist x nprobe for IVF-SQ and export "
    ...
    parser.add_argument(
        "--output-xlsx",
        default="sampling_ivfsq_param_sweep_results.xlsx",
        help="Output Excel (.xlsx) path.",
    )
    parser.add_argument(
        "--index-type",
        default="IVF_SQ8",
        help="Milvus IVF-SQ index type, e.g. IVF_SQ8 or IVF_SQ8H.",
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
...
def make_run_tag(sample_ratio: float, nlist: int, nprobe: int, seq: int) -> str:
    ...
    return f"sample-ivfsq-r{safe_ratio}-nl{nlist}-np{nprobe}-{seq:04d}-{ts}"
...
def main() -> None:
    ...
    nprobe_values = list(range(10, 51, 10))
    ...
    def append_fail(
        ...
        rows.append(
            MetricRow(
                experiment_group="ivfsq_full_factorial",
    ...
    try:
        ...
        print(f"Completed IVF-SQ full-factorial runs: {len(rows)}/{total_runs}")
    finally:
