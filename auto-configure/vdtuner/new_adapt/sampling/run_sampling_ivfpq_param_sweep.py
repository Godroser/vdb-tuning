#!/usr/bin/env python3
"""Full-factorial sweep for IVF_PQ parameters with sampling benchmark."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep SAMPLE_RATIO x nlist x nprobe x m x nbits for IVF_PQ and "
    ...
    parser.add_argument(
        "--output-xlsx",
        default="sampling_ivfpq_param_sweep_results.xlsx",
...
def set_ivfpq_params(
    ...
    upload_params["index_type"] = "IVF_PQ"
...
def make_run_tag(
    ...
    return (
        f"sample-ivfpq-r{safe_ratio}-nl{nlist}-np{nprobe}-m{m}-nb{nbits}-"
        f"{seq:04d}-{ts}"
    )
...
def main() -> None:
    ...
    sample_ratio_values = [0.10, 0.08, 0.05, 0.03, 0.01]
    nlist_values = list(range(50, 501, 150))
    nprobe_values = list(range(10, 51, 25))
    m_values = [4, 20]
    nbits_values = [4, 8]
    total_runs = (
    ...
    def append_fail(
        ...
        rows.append(
            MetricRow(
                experiment_group="ivfpq_full_factorial",
    ...
    try:
                ...
                for nprobe in nprobe_values:
                    for m in m_values:
                        for nbits in nbits_values:
                            run_index += 1
                            print(
                                f"[{run_index}/{total_runs}] SAMPLE_RATIO={ratio}, "
                                f"nlist={nlist}, nprobe={nprobe}, m={m}, nbits={nbits}"
                            )
                            ...
                            try:
                                ...
                                rows.append(
                                    MetricRow(
                                        ...
                                        original_mean_precisions=original.get(
                                            "mean_precisions"
                                        ),
                                        sampled_rps=sampled.get("rps"),
                                        sampled_p95_time=sampled.get("p95_time"),
                                        sampled_mean_precisions=sampled.get(
                                            "mean_precisions"
                                        ),
                                        run_tag=run_tag,
        ...
        print(f"Completed IVF_PQ full-factorial runs: {len(rows)}/{total_runs}")
    finally:
