#!/usr/bin/env python3
"""Compute ranking loss while excluding rows with nlist=500."""
...
if "nlist" not in df.columns:
    raise ValueError("Missing required column: nlist")

excluded_count = int((df["nlist"] == EXCLUDED_NLIST).sum())
df = df[df["nlist"] != EXCLUDED_NLIST].copy()
...
print(f"excluded nlist: {EXCLUDED_NLIST} (rows removed: {excluded_count})")
