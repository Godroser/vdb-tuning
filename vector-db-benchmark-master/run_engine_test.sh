...
VENV_PATH=${VENV_PATH:-"/data/dzh/vdtuner-gpu-env"}
...
# 确保没有残留容器
docker rm -f milvus-standalone-gpu milvus-etcd-gpu milvus-minio-gpu 2>/dev/null || true
sleep 2
...
while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
    # 检查容器是否在运行
    if ! docker ps | grep -q milvus-standalone-gpu; then
        echo "    ⚠️  milvus-standalone-gpu 容器未运行，检查日志..."
        docker logs milvus-standalone-gpu --tail 20 2>&1 | grep -E "error|fatal|panic" | head -5 || true
        echo "    ❌ 容器启动失败，退出"
        exit 1
    fi
    
    # 检查健康状态
    HEALTH=$(docker inspect --format='{{.State.Health.Status}}' milvus-standalone-gpu 2>/dev/null || echo "none")
    ...
    # 检查容器是否退出
    STATUS=$(docker inspect --format='{{.State.Status}}' milvus-standalone-gpu 2>/dev/null || echo "unknown")
    if [ "$STATUS" = "exited" ]; then
        echo "    ❌ 容器已退出，退出码: $(docker inspect --format='{{.State.ExitCode}}' milvus-standalone-gpu 2>/dev/null || echo 'unknown')"
        echo "    最后 30 行日志:"
        docker logs milvus-standalone-gpu --tail 30 2>&1
        exit 1
    fi
